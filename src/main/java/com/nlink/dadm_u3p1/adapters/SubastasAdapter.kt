...
) : RecyclerView.Adapter<SubastasAdapter.SubastasVH>() {
        ...
        fun bind(s: Subasta) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                if(LocalDate.now().isAfter(fechaVencimiento) ) {
                    ...
                    println("Subasta disponible")
                    btnSubastar.setOnClickListener {
                        val intent = Intent(ctx, PujarActivity::class.java)
                        intent.putExtra("producto", s)
